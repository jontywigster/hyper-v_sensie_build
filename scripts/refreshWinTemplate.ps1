...
if ($vm) {
    ...
    if ($refresh -eq 'y') {
        & .\scripts\removeVM.ps1 -vmName $vmName | Out-Null
    }
    else { 
        return "$vmName" 
    }
...
$vm = & .\scripts\createVM.ps1 -vmName $vmName -vmFolder "$vmFolder" -vhdx "$sourceVHDX" -vSwitch "$vSwitch" -notes ("created:" + (Get-Date -Format "dd/MM/yy")) -bWindows $true -bStartVM $false  | Out-Null
. .\scripts\getEnvVars.ps1
$templateVHDX = Join-Path $vmFolder $(Split-Path $sourceVHDX -Leaf)
& .\scripts\injectWinUnattend.ps1 -vhdx "$templateVHDX" -os $edition -hostname $vmName -adminPwd "$WIN_TEMPLATE_ADMIN_PWD" | Out-Null
