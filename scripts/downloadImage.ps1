...
$overrideNRVar = "URL_$os"
if (Test-Path -Path Variable:$overrideNRVar) {
  $downloadURL = $(Get-Variable -Name $overrideNRVar -ValueOnly)
  $versionVar = "VERSION_$os"
  if (Test-Path -Path Variable:$overrideNRVar) {
    $relProps = [PSCustomObject]@{ 
      url = $downloadURL
      relToNum = $(Get-Variable -Name $versionVar -ValueOnly) 
    }
  }
  else {
    throw "manaul url set for $os but no version set"
  }
...
if ($windows) {
  $downloadFile = [string] $relProps.relToNum + "-source.iso"
  $outputVHDX = $relProps.relToNum + "-" + $os + "-source.vhdx"
}
else {
